...
public class AsyncListenersTest {
  ...
  public void testListenersForSuccessfulCompletion() throws Throwable {
    ...
    // When
    Recurrent.with(new RetryPolicy().retryWhen(false)).with(executor).with(listeners).get(callable);
  ...
  public void testListenersForFailureCompletion() throws Throwable {
    ...
    // When
    Recurrent.with(new RetryPolicy().retryWhen(false).withMaxRetries(3)).with(executor).with(listeners).get(callable);
