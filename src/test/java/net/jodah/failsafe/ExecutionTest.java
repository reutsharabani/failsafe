...
public class ExecutionTest {
	...
	public void testCanRetryForResult() {
		...
		Execution exec = new Execution(new RetryPolicy.Builder().retryWhen(null).build());
		...
		// Given 2 max retries
		exec = new Execution(new RetryPolicy.Builder().retryWhen(null).withMaxRetries(2).build());
	...
	public void testCanRetryOn() {
		...
		Execution exec = new Execution(new RetryPolicy.Builder().retryOn(IllegalArgumentException.class).build());
		...
		// Given 2 max retries
		exec = new Execution(new RetryPolicy.Builder().withMaxRetries(2).build());
	...
	public void testComplete() {
		// Given
		Execution exec = new Execution(new RetryPolicy.Builder().build());
	...
	public void testIsComplete() {
		...
		RetryPolicy retryPolicy = new RetryPolicy.Builder().retryOn(IllegalStateException.class).build();
	...
	public void shouldAdjustWaitTimeForBackoff() {
		Execution exec = new Execution(new RetryPolicy.Builder().withBackoff(1, 10, TimeUnit.NANOSECONDS).build());
	...
	public void shouldAdjustWaitTimeForMaxDuration() throws Throwable {
		Execution exec = new Execution(new RetryPolicy.Builder().withDelay(49, TimeUnit.MILLISECONDS)
				.withMaxDuration(50, TimeUnit.MILLISECONDS).build());
		Thread.sleep(10);
		assertTrue(exec.canRetryOn(e));
		assertTrue(
				exec.getWaitTime().toNanos() < TimeUnit.MILLISECONDS.toNanos(50) && exec.getWaitTime().toNanos() > 0);
	}

	public void shouldSupportMaxDuration() throws Exception {
		Execution exec = new Execution(new RetryPolicy.Builder().withMaxDuration(100, TimeUnit.MILLISECONDS).build());
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		Thread.sleep(100);
		assertFalse(exec.canRetryOn(e));
	...
	public void shouldSupportMaxRetries() throws Exception {
		Execution exec = new Execution(new RetryPolicy.Builder().withMaxRetries(3).build());
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		assertFalse(exec.canRetryOn(e));
	...
	public void shouldGetWaitMillis() throws Throwable {
		Execution exec = new Execution(new RetryPolicy.Builder().withDelay(100, TimeUnit.MILLISECONDS)
				.withMaxDuration(101, TimeUnit.MILLISECONDS).retryWhen(null).build());
