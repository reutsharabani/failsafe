...
public class ExecutionTest {
	...
	public void testCanRetryForResult() {
		...
		Execution exec = new Execution(RetryPolicy.newBuilder().retryWhen(null).build());
		...
		// Given 2 max retries
		exec = new Execution(RetryPolicy.newBuilder().retryWhen(null).withMaxRetries(2).build());
	...
	public void testCanRetryOn() {
		...
		Execution exec = new Execution(RetryPolicy.newBuilder().retryOn(IllegalArgumentException.class).build());
		...
		// Given 2 max retries
		exec = new Execution(RetryPolicy.newBuilder().withMaxRetries(2).build());
	...
	public void testComplete() {
		// Given
		Execution exec = new Execution(RetryPolicy.newBuilder().build());
	...
	public void testIsComplete() {
		...
		RetryPolicy retryPolicy = RetryPolicy.newBuilder().retryOn(IllegalStateException.class).build();
	...
	public void shouldAdjustWaitTimeForBackoff() {
		Execution exec = new Execution(RetryPolicy.newBuilder().withBackoff(1, 10, TimeUnit.NANOSECONDS).build());
	...
	public void shouldAdjustWaitTimeForMaxDuration() throws Throwable {
		Execution exec = new Execution(RetryPolicy.newBuilder().withDelay(49, TimeUnit.MILLISECONDS)
	...
	public void shouldSupportMaxDuration() throws Exception {
		Execution exec = new Execution(RetryPolicy.newBuilder().withMaxDuration(100, TimeUnit.MILLISECONDS).build());
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		Thread.sleep(105);
		assertFalse(exec.canRetryOn(e));
	...
	public void shouldSupportMaxRetries() throws Exception {
		Execution exec = new Execution(RetryPolicy.newBuilder().withMaxRetries(3).build());
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		assertTrue(exec.canRetryOn(e));
		assertFalse(exec.canRetryOn(e));
	...
	public void shouldGetWaitMillis() throws Throwable {
		Execution exec = new Execution(RetryPolicy.newBuilder().withDelay(100, TimeUnit.MILLISECONDS)
